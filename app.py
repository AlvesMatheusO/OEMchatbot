...
def build_detalhe(produto: dict) -> str:
    ...
    oem = (
        produto.get("codigo_oem")
        or produto.get("oem_montadora")
        or "---"
    )

    sku = (
        produto.get("sku_autoflex")
        or produto.get("ref_autoflex")
        or "---"
    )

    mon = (
        produto.get("montadora")
        or produto.get("fabricante")
        or "---"
    )

    apl = (
        produto.get("aplicacao")
        or produto.get("veiculo")
        or ""
    )
    ...
    if cr:
        ...
        marcas = [
            ...
            if (
                k not in ("sku_autoflex", "descricao")
                and v
...
def build_lista(produtos: list) -> str:
    ...
    for i, p in enumerate(produtos[:5], 1):
        ...
        oem = (
            p.get("codigo_oem")
            or p.get("oem_montadora")
            or "---"
        )

        sku = (
            p.get("sku_autoflex")
            or p.get("ref_autoflex")
            or "---"
        )
...
def chat():
    ...
    if msg_low.isdigit():
        ...
        if (
            ...
            return jsonify({
                "response": build_detalhe(produto)
            })
        ...
        if (
            ...
            if msg_low == "2":
                ...
                simil = search_oem_parts_by_term(
                    desc,
                    modelo
                )
                ...
                simil = [
                    ...
                    if (
                        s.get("ref_autoflex") != sku
                        and
                        s.get("sku_autoflex") != sku
                    )
                ]
    ...
    if modelo:
        ...
        fitments = search_vehicle_fitment(
            modelo,
            ano
        )
    ...
    if not produtos:
        ...
        produtos = search_oem_parts_by_term(
            termo,
            modelo,
            fabricante
        )
    ...
    if produtos:
        ...
        if len(produtos) == 1:
            ...
            return jsonify({
                "response": build_detalhe(produtos[0])
            })
    ...
    externos = external_search(
        modelo=modelo,
        ano=ano,
        peca=peca or message
    )
    ...
    if externos:
        ...
        for i, item in enumerate(externos[:5], 1):
            ...
            txt += (
                ...
                f"Referência: "
                f"{item.get('referencia') or '-'}\n\n"
            )

        txt += (
            "\n⚠️ Resultado vindo de "
            "catálogo externo TecDoc."
        )
